...
const NIFMONY = path.join(ROOT, "bin", "nifmony");
const EX = path.join(ROOT, "examples");

function nifmony(args) {
  const r = cp.spawnSync("node", [NIFMONY, ...args], { encoding: "utf8" });
...
for (const [file, entry, args, want] of NATIVE) {
  ...
  check(`${entry}(${args.join(",")})`, nifmony(a).out, want);
}

// --- interpreter (nifi): run whole program, assert echoed output ----------
console.log("interpret (nifparser → sem → nifi):");
check("interp demo.nim output", nifmony(["interp", path.join(EX, "demo.nim")]).out, "6765\n3628800\ntrue");

// --- provenance: the user module is parsed by OUR nifparser ---------------
console.log("provenance:");
const nif = nifmony(["nif", path.join(EX, "compute.nim"), "-v"]);
